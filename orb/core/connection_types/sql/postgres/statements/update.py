        ...
        for column in changes:
            ...
            if column.test_flag(column.Flags.I18n):
                for record_locale, value in record.get(column, locale='all').items():
                    ...
                    data[value_key] = engine.get_database_value(column, 'Postgres', value)
                ...
                value_key = '{0}_{1}'.format(column.field(), os.urandom(4).encode('hex'))
                data[value_key] = engine.get_database_value(column, 'Postgres', record.get(column))
...
PSQLStatement.registerAddon('UPDATE', UPDATE())
