...
class Column(object):
    ...
    def default(self, resolve=False):
        ...
        if not resolve:
            ...
            if type(default) == bool:
                ...
                return nativestring(default) in ('True', '1')
    ...
    def fieldName(self, language=None):
        ...
        if self.isTranslatable():
            if language is None:
                language = orb.Orb.instance().language()
    ...
    def languages(self):
        ...
        if self.isTranslatable():
            return orb.Orb.instance().languages()
        else:
            return [orb.Orb.instance().language()]
    ...
    def setLanguages(self, languages):
        ...
        None is provided, then the global Orb languages will be used.
    ...
    def toXml(self, xparent):
        ...
        xcolumn.set('indexed', nativestring(self.indexed()))
        xcolumn.set('indexCached', nativestring(self.indexCached()))
        xcolumn.set('indexCachedExpires', nativestring(self.indexCachedExpires()))
    
        if self.default() is not None:
            xcolumn.set('default', nativestring(self.default()))
        
        # store additional options
        xcolumn.set('maxlen', nativestring(self.maxlength()))
        
        # store flags
        for flag in Column.Flags.keys():
            key = flag[0].lower() + flag[1:]
            xcolumn.set(key, nativestring(self.testFlag(Column.Flags[flag])))
        ...
        if self.reference():
            ...
            xrelation.set('reversed', nativestring(self.isReversed()))
            xrelation.set('reversedName', self.reversedName())
            xrelation.set('removedAction', nativestring(self.referenceRemovedAction()))
            xrelation.set('cached', nativestring(self.reversedCached()))
            xrelation.set('expires', nativestring(self.reversedCacheExpires()))
        ...
        if validator:
            if not value in (str, unicode):
                try:
                    ...
                    return (False, nativestring(errors.ValidationError(self, value)))
    ...
    def valueFromString(self, value, extra=None, db=None):
        ...
        if coltype == ColumnType.Date:
            ...
            return nativestring(value).lower() == 'true'
