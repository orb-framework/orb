...
class Table(object):
    ...
    def __format__(self, format_spec):
        ...
        if format_spec == 'primaryKey':
            return nativestring(self.primaryKey())
        else:
            column = self.schema().column(format_spec)
            if column:
                return nativestring(self.recordValue(format_spec))
    ...
    def __cmp__(self, other):
        ...
        :return     -1 || 0 || 1
        """
        return cmp(nativestring(self), nativestring(other))
    ...
    def commit(self, *args, **kwds):
        ...
        if db is None:
            logger.error('No database defined for %s, cannot commit', nativestring(self))
            return ('error', 'No database defined for {}'.format(self))
        
        # grab the backend
        backend = db.backend()
        if not backend:
            logger.error('No backend found for %s, cannot commit', nativestring(db))
        ...
        try:
            ...
            if options.throwErrors:
                ...
                results = ('errored', nativestring(err))
                print err
    ...
    def recordValue(self,
        ...
        if not column:
            ...
            raise errors.ColumnNotFoundWarning(table, columnName)
        ...
        if column.isTranslatable():
            ...
            # return the current language
                ...
                language = Orb.instance().language()
    ...
    def setRecordValue(self,
        ...
        column = self.schema().column(columnName)
        if not column:
            raise errors.ColumnNotFoundWarning(column, self.schema().name())
    ...
    def getDatabase(cls):
        ...
        if isinstance(db, orb.Database):
            ...
            return orb.Orb.instance().database(db)
        else:
    ...
    def inflateRecord(cls, values, default=None, db=None):
        ...
        if column and column.name() in values:
            ...
            if morph:
                morph_name = nativestring(morph(values[column.name()], db=db))
    ...
    def proxyColumn(cls, name):
        ...
        """
        return getattr(cls, '_%s__proxies' % cls.__name__, {}).get(nativestring(name))
    ...
    def searchThesaurus(cls):
        ...
        return getattr(cls, key, Orb.instance().searchThesaurus())
