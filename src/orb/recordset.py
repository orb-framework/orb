...
from orb.common import RemovedAction, SearchMode, DeleteFlags
...
class RecordSet(object):
    ...
    def all(self, **options):
        ...
        if key in self._all:
            ...
            results = None
            if cache is not None and orb.Orb.instance().isCachingEnabled():
        ...
        for table, query in breakdown.items():
            ...
            try:
                ...
                if dbopts.throwErrors:
                    ...
                    logger.debug('Backend error ocurred.\n%s', err)
    ...
    def search(self,
        ...
        if not type(search_terms) in (list, tuple):
            terms, column_query = Query.fromSearch(nativestring(search_terms),
                                                   mode,
                                                   thesaurus=thesaurus)
        else:
    ...
    def values(self, columns, **options):
        ...
        if lang is None:
            lang = orb.Orb.instance().language()
