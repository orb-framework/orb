...
class Quickbase(Connection):
    ...
    def queryCommand( self, schema, query ):
        ...
        if ( not col ):
            warn = errors.ColumnNotFoundWarning(colname, tableName)
            logger.warning(warn)
            return ''
    ...
    def request(self,
        ...
        # debug the lookup
        logger.debug(url)
        logger.debug(nativestring(headers))
        ...
        try:
            ...
            try:
                ...
                response = None
            raise ConnectionError(nativestring(error))
        except urllib2.URLError as error:
            raise ConnectionError(nativestring(error))
        except Exception as error:
            raise UnknownError(nativestring(error))
        ...
        try:
            ...
            raise ResponseError(nativestring(error))
    ...
    def buildRequest(**fields):
        ...
        def add_sub_element(field, value):
            if isinstance(value, tuple):
                attrib, value = value
                attrib = dict((k, nativestring(v)) for k, v in attrib.iteritems())
            else:
                attrib = {}
            sub_element = ElementTree.SubElement(request, field, **attrib)
            sub_element.text = nativestring(value)
