...
"""
Defines the base abstract AbstractSQL connection for all SQL based
connection backends. """
...
# flag commands
orb.Column.Flags.Required:      u'NOT NULL',
orb.Column.Flags.Unique:        u'UNIQUE',
orb.Column.Flags.AutoIncrement: u'AUTO_INCREMENT',
...
DEFAULT_TYPE_MAP = {
    orb.ColumnType.Bool:        u'BOOL',
        
    orb.ColumnType.Decimal:     u'DECIMAL UNSIGNED',
    orb.ColumnType.Double:      u'DOUBLE UNSIGNED',
    orb.ColumnType.Integer:     u'INT UNSIGNED',
    orb.ColumnType.Enum:        u'INT UNSIGNED',
    orb.ColumnType.BigInt:      u'BIGINT UNSIGNED',

    orb.ColumnType.ForeignKey:  u'BIGINT UNSIGNED',

    orb.ColumnType.Datetime:             u'DATETIME',
    orb.ColumnType.Date:                 u'DATE',
    orb.ColumnType.Time:                 u'TIME',
    orb.ColumnType.DatetimeWithTimezone: u'TIMESTAMP',

    orb.ColumnType.String:      u'VARCHAR',
    orb.ColumnType.Color:       u'VARCHAR',
    orb.ColumnType.Email:       u'VARCHAR',
    orb.ColumnType.Password:    u'VARCHAR',
    orb.ColumnType.Url:         u'VARCHAR',
    orb.ColumnType.Filepath:    u'VARCHAR',
    orb.ColumnType.Directory:   u'VARCHAR',
    orb.ColumnType.Text:        u'TEXT',
    orb.ColumnType.Xml:         u'TEXT',
    orb.ColumnType.Html:        u'TEXT',

    orb.ColumnType.ByteArray:   u'VARBINARY',
    orb.ColumnType.Yaml:        u'TEXT',
    orb.ColumnType.Query:       u'TEXT',
    orb.ColumnType.Pickle:      u'BLOB',
    orb.ColumnType.Dict:        u'BLOB',
    orb.ColumnType.Image:       u'BLOB'
}

DEFAULT_LENGTHS = {
    orb.ColumnType.String:    256,
    orb.ColumnType.Color:     25,
    orb.ColumnType.Email:     256,
    orb.ColumnType.Password:  256,
    orb.ColumnType.Url:       500,
    orb.ColumnType.Filepath:  500,
    orb.ColumnType.Directory: 500
}
...
class SqlColumnEngine(orb.ColumnEngine):
    ...
    def addCommand(self, column, language=None):
        ...
        """
        cmd = u''
        if column.testFlag(orb.Column.Flags.Primary):
    ...
    def queryCommand(self,
        ...
        elif op in (orb.Query.Op.Startswith, orb.Query.Op.Endswith):
            native = projex.text.decoded(value)
            if op == orb.Query.Op.Startswith:
                value = u'^{0}.*$'.format(native)
            else:
                value = u'^.*{0}$'.format(native)
        ...
        elif op in (orb.Query.Op.IsIn, orb.Query.Op.IsNotIn):
            value = tuple(value)
            if not value:
                return u'', {}
        
        # generate a basic query
        sqlop = self.command(orb.Query.Op[op])
        if not sqlop:
...
class SqlSchemaEngine(orb.SchemaEngine):
    ...
    def insertCommand(self,
        ...
        for column in schema.columns(recurse=is_oo,
            ...
            if autoincrement and column.testFlag(orb.Column.Flags.AutoIncrement):
                continue
    ...
    def queryCommand(self, schemas, query, setup=None):
        ...
        for column, value in dict(zip(column, value)).items():
            ...
            for lang in langs:
                ...
                elif value == orb.Query.ALL:
                    continue
                
                # lookup all NOT NULL instances of a record
                elif value == orb.Query.NOT_EMPTY or \
                     (value == None and op == orb.Query.Op.IsNot):
                    commands.append(u'{0} IS NOT NULL'.format(sqlfield))
                    continue
                
                # lookup all NULL instances
                elif value == orb.Query.EMPTY or (value == None and op == orb.Query.Op.Is):
...
class AbstractSQL(orb.Connection):
    """ 
    ...
    def __init__(self,
        ...
        super(AbstractSQL, self).__init__(database)
        ...
        for typ, engine_type in orb.system.customEngines(plug):
    ...
    def _open(self, db):
        ...
        the main open method for the AbstractSQL class.
    ...
    def createTable(self, schema, options):
        ...
        if schema.isAbstract():
            name = schema.name()
            log.debug('%s is an abstract table, not creating' % name)
            return False
        ...
        log.info('Created %s table.' % schema.tableName())
        return True
    ...
    def disableInternals(self):
        ...
        :sa     enableInternals
        """
        super(AbstractSQL, self).disableInternals()
    ...
    def enableInternals(self):
        ...
        super(AbstractSQL, self).enableInternals()
    ...
    def updateTable(self, schema, options):
        ...
        if options.dryRun:
            ...
            log.info(u'Updated {0} table: added {1}'.format(*opts))
